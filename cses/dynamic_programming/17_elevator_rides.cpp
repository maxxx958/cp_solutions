...
void solve(){
	...
	vii dp((1<<n), {inf, inf}); // how many rides so far, how much weight so far in last ride
	dp[0] = {0, x};
	for(int m = 1; m < (1 << n); m++){
		for(int i = 0; i < n; i++){
			if((m & (1 << i)) == 0) continue;
			if(dp[m - (1 << i)].nd + w[i] <= x){
				dp[m] = min(dp[m], {dp[m - (1 << i)].st, dp[m - (1 << i)].nd + w[i]});
			}else{
				dp[m] = min(dp[m], {dp[m - (1 << i)].st + 1, w[i]});
			}
		}
	}
	cout << dp[(1 << n) - 1].st;
